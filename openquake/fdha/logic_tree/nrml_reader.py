...
def parse(xml_path: str | Path) -> LogicTreeSpec:
    ...
    if lt.findall("nrml:logicTreeBranchingLevel", NRML_NS):
        raise ValueError(
            "<logicTreeBranchingLevel> is obsolete for FDHA logic trees; "
            "put <logicTreeBranchSet> directly under <logicTree>")
    for bs in lt.findall("nrml:logicTreeBranchSet", NRML_NS):
        ...
        branching_levels.append(BranchingLevel(
            branching_level_id="",
            branch_sets=(BranchSet(
                branch_set_id=bs_id,
