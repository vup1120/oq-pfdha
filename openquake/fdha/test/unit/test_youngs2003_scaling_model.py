...
def test_instantiate_model_propagates_constructor_error():
    """_instantiate_model must not degrade a bad config to warning+None."""
    from openquake.fdha.calc.calculators import BaseFaultRuptureCalculator
    cfg = {"type": "Youngs2003PrimaryFD",
           "parameters": {"scaling_model": "LEONARD2010"}}
    with pytest.raises(ValueError, match="scaling_model"):
        BaseFaultRuptureCalculator._instantiate_model(cfg)


def test_instantiate_model_rejects_unknown_class():
    from openquake.fdha.calc.calculators import BaseFaultRuptureCalculator
    with pytest.raises(ValueError, match="Unknown FDHA model class"):
        BaseFaultRuptureCalculator._instantiate_model(
            {"type": "NoSuchModel2099", "parameters": {}})


def test_instantiate_model_good_config_still_works():
    from openquake.fdha.calc.calculators import BaseFaultRuptureCalculator
    cfg = {"type": "Youngs2003PrimaryFD",
           "parameters": {"scaling_model": "WC1994", "n_sigma": 6.0}}
    model = BaseFaultRuptureCalculator._instantiate_model(cfg)
    assert isinstance(model, Youngs2003PrimaryFD)
    assert model.scaling_model == "WC1994"
