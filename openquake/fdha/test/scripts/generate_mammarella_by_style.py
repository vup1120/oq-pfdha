#!/usr/bin/env python3
"""
Mammarella et al. (2024) validation - Grouped by Fault Style.
Each subplot shows all 3 MSRs for one fault style.
"""
...
def generate_plot():
    print("=" * 70)
    print("Mammarella et al. (2024) - Grouped by Fault Style")
    print("=" * 70)
    ...
    msr_config = {
        0: {'name': 'WC94', 'color': '#1f77b4'},
        1: {'name': 'LSRF', 'color': '#ff7f0e'},
        2: {'name': 'TMG', 'color': '#2ca02c'},
    }
    ...
    for idx, (style, style_name) in enumerate(styles_config.items()):
        ax = axes[idx]
        print(f"\n{style_name}:")
        
        for msr, config in msr_config.items():
            ref_key = (msr, style)
            ...
            ax.plot(ref_mags, ref_probs, color=config['color'], linestyle='--',
                    linewidth=2.5, label=f'{config["name"]} (ref)')
            ...
            print(f"  MSR={msr} ({config['name']:4s}): max rel diff = {rel_diff:.2e}")
        
        ax.set_xlabel('Magnitude, Mw', fontweight='bold')
        if idx == 0:
            ax.set_ylabel('P(Surface Rupture)', fontweight='bold')
        ax.set_title(style_name, fontweight='bold')
        ax.set_xlim([5.0, 8.0])
        ax.set_ylim([0, 1.05])
        ax.grid(True, alpha=0.3)
        ax.legend(loc='upper left', fontsize=9, ncol=2)
    
    fig.suptitle('Mammarella et al. (2024) Validation - Grouped by Fault Style\n'
                 'Dashed: CPSR.m reference | Solid+△: our work',
                 fontsize=13, fontweight='bold', y=1.02)
    
    plt.tight_layout()
    OUTPUT_DIR.mkdir(exist_ok=True)
    
    output_path = OUTPUT_DIR / "mammarella2024_by_style.png"
    plt.savefig(output_path, dpi=600, bbox_inches='tight', facecolor='white')
    print(f"\nSaved: {output_path}")
    
    plt.savefig(OUTPUT_DIR / "mammarella2024_by_style.pdf", bbox_inches='tight', facecolor='white')
    plt.close(fig)
    return 0
