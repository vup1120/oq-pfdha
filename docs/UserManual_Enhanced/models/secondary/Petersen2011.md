...
| --- | --- | --- | --- | --- | --- | --- |
| `cell_size` | integer | meters | `25` | `25`, `50`, `100`, `150`, `200` | No | Size of the cell/pixel for distributed rupture modeling. Different cell sizes have different regression coefficients. |

**Note:** While the cell_size parameter is accepted, the current implementation uses a single regression equation (Page 818, Eqn 18) for all cell sizes. The cell size-specific coefficients from Table 4 are defined in the code but not currently used in the regression calculation.
...
- **Cell size:** The `cell_size` parameter is accepted (25, 50, 100, 150, or 200 m) but the current implementation uses a single regression equation regardless of cell size. Invalid cell sizes may raise errors.
